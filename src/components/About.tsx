...
export default function About() {
  ...
  // Torch spotlight over the notebook — reuses the same mx/my tracking
  // already driving the tilt above, so no extra event listeners or
  // re-renders are introduced. This does not touch or replace the site's
  // existing global custom cursor component; it's a light source, not a
  // cursor. Always-on (not just while hovering) so the page reads as
  // deliberately, dramatically lit even before the pointer ever moves —
  // it just brightens and re-centers on the pointer once it does.
  const spotlightOpacity = useMotionValue(0.55);
  ...
  const spotlightBg = useMotionTemplate`radial-gradient(560px circle at ${spotlightX} ${spotlightY}, rgba(255,255,255,0.4), rgba(29,111,235,0.08) 45%, transparent 70%)`;
  ...
  return (
    <section
      ref={sectionRef}
      id="about"
      className="relative overflow-hidden section-wrap max-w-full py-12 sm:py-16 md:py-16 lg:py-16 bg-[#050507]"
      aria-label="About — My Story"
    >
      {/* subtle blueprint grid backdrop — kept faint enough that it reads as
          texture in the dark rather than a light-mode leftover */}
      <div
      ...
      {/* ambient motes — on a black stage these read as drifting dust caught
          in the notebook's own light, rather than paper dust on white */}
        ...
        {dust.map((_, i) => (
          <motion.span
            ...
            style={{
              ...
              width: i % 3 === 0 ? 3 : 2,
              height: i % 3 === 0 ? 3 : 2,
              boxShadow: "0 0 6px 1px rgba(29,111,235,0.6)",
            }}
            animate={rm ? {} : { y: [0, -22, 0], opacity: [0.15, 0.65, 0.15] }}
      ...
      <div className="container-tight relative z-10 max-w-full">
        ...
        <div className="hidden lg:block relative max-w-[1200px] mx-auto">
          ...
          >
            ...
            {/* permanent vignette — darkens toward the page edges so the
                notebook reads as a single lit object on a dark stage,
                exactly like the reference: light pools where the story is,
                everything else recedes into shadow. This sits underneath
                the pointer-tracked highlight above, which only adds warmth
                and re-centers the brightest point on the cursor. */}
            <div
              aria-hidden="true"
              className="absolute inset-0 pointer-events-none z-30"
              style={{
                background:
                  "radial-gradient(120% 90% at 50% 32%, transparent 35%, rgba(0,0,0,0.16) 75%, rgba(0,0,0,0.34) 100%)",
              }}
            />

            {/* soft cursor-follow light highlight — always faintly present
                (see spotlightOpacity above), brightens and re-centers on
                the pointer while it's over the notebook */}
            {!rm && (
              <motion.div
                aria-hidden="true"
              ...
              >
                ...
                >
                  ...
                  {/* ═══════════ PAGE 3 — closing spread, left half ═══════════
                      The notebook's last pages turn to solid black and one
                      final, quiet beat: just the signature. Everything else
                      on this page has been building toward this — it's the
                      one moment that gets to be almost empty. */}
                  <NotebookPage dark className="px-6 py-6 sm:px-10 sm:py-8 lg:pr-14 lg:pl-12 flex items-center justify-end">
                    <div className="absolute top-6 left-6 right-6 sm:top-10 sm:left-10 sm:right-10 flex items-center justify-between text-[10px] sm:text-xs font-mono uppercase tracking-[0.2em] text-white/25">
                    ...
                    <motion.p
                      ...
                      className="font-hand text-white text-6xl sm:text-7xl leading-none"
                  ...
                  <NotebookPage dark className="px-6 py-6 sm:px-10 sm:py-8 lg:pl-14 lg:pr-12 flex flex-col items-start justify-center">
                    <div className="absolute top-6 left-6 right-6 sm:top-10 sm:left-10 sm:right-10 flex items-center justify-end text-[10px] sm:text-xs font-mono uppercase tracking-[0.2em] text-white/25">
                      <span>Thank You</span>
                    </div>
                    <motion.p
                      initial={{ opacity: 0, y: rm ? 0 : 16 }}
                      ...
                      className="font-hand text-white text-6xl sm:text-7xl leading-none mb-8"
                      style={{ textShadow: "0 0 40px rgba(255,255,255,0.15)" }}
                    >
                      Paharia
                    </motion.p>
                    <motion.blockquote
                      ...
                      className="text-white/50 italic text-sm sm:text-base leading-relaxed max-w-xs mb-6"
                    >

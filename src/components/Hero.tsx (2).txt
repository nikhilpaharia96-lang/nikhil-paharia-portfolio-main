...
import { SiReact, SiNodedotjs, SiJavascript, SiTailwindcss, SiFigma } from "react-icons/si";
...
export default function Hero() {
  ...
  // translateY, no scale tied to scrollYProgress. It's rendered as a plain,
  // non-motion, absolutely positioned layer below (see "Fixed background"
  // comment). Depth/parallax instead comes from MOUSE movement only (below),
  // which never fights Lenis/scroll and costs nothing while the page is static.

  /* ── Mouse parallax + cursor spotlight — CSS-custom-property driven, not React
     state. A single rAF-throttled mousemove listener writes directly to the
     section's inline style; every layer just reads the inherited custom
     properties in its own `transform`/`background`. Zero re-renders, zero
     layout reads, GPU-only compositing. Disabled entirely under
  ...
  useEffect(() => {
    ...
    const apply = () => {
      raf = 0;
      el.style.setProperty("--mx", String((pendingX - 0.5) * 2)); // -1 .. 1
      el.style.setProperty("--my", String((pendingY - 0.5) * 2)); // -1 .. 1
      el.style.setProperty("--spot-x", `${pendingX * 100}%`);
      el.style.setProperty("--spot-y", `${pendingY * 100}%`);
    };
  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-center overflow-hidden w-full max-w-full section-wrap" id="home">
      ...
      <style>{`
        ...
        /* ── Mouse-driven parallax depth layers (never scroll-linked) ──
           --mx/--my are set on the <section> root, range -1..1. Each layer
           just multiplies by its own distance factor and eases with a CSS
           transition (Apple/Linear-style smoothing, no spring library needed
           for something this subtle). */
        .parallax-slow, .parallax-mid, .parallax-fast {
          transition: transform 0.6s cubic-bezier(0.22, 1, 0.36, 1);
          will-change: transform;
        }
        .parallax-slow { transform: translate3d(calc(var(--mx, 0) * 4px), calc(var(--my, 0) * 3px), 0); }
        .parallax-mid  { transform: translate3d(calc(var(--mx, 0) * 10px), calc(var(--my, 0) * 7px), 0); }
        .parallax-fast { transform: translate3d(calc(var(--mx, 0) * 18px), calc(var(--my, 0) * 12px), 0); }
        @media (prefers-reduced-motion: reduce) {
          .parallax-slow, .parallax-mid, .parallax-fast { transform: none !important; }
        }

        /* ── Soft press feedback on the CTA buttons — a lightweight CSS
           approximation of a ripple (an expanding radial highlight from the
           center on :active), no click-handler/JS state needed */
        .btn-press { position: relative; }
        .btn-press::after {
        ...
        .btn-press:active::after { opacity: 1; transform: scale(1.3); transition: none; }
  ...
  className="font-bold"
>
  <SplitText type="chars" delay={0.25} textClassName="gradient-text">Nikhil Paharia</SplitText>
</h1>
            ...
            >
              <Magnetic range={60} strength={0.35} scaleHover={1.03}>
                <a
                  href="#contact"
                  className="relative overflow-hidden cta-pulse btn-press inline-flex items-center justify-center gap-2 bg-primary text-white font-bold px-8 rounded-full shadow-lg hover:shadow-[0_0_30px_rgba(29,111,235,0.45)] transition-shadow duration-300"
              ...
              <Magnetic range={60} strength={0.35} scaleHover={1.03}>
                <a
                  href="#projects"
                  className="relative overflow-hidden btn-press inline-flex items-center justify-center gap-2 bg-white/80 backdrop-blur border border-blue-200 text-primary font-bold px-8 rounded-full hover:bg-blue-50 transition-colors duration-300 mr-3 sm:mr-4"
                  style={{ fontSize: 'clamp(0.85rem, 2vw, 1rem)', height: '56px' }}
                >
          ...
          >
            <Tilt maxRotate={5} glowColor="#2563EB" glowOpacity={0.1} glowSize={360} className="mx-auto">
            <div className="relative mx-auto" style={{ width: 'clamp(180px, 52vw, 380px)', height: 'clamp(220px, 63vw, 460px)' }}>
              ...
              {[
                ...
                { Icon: FaGithub,      color: "text-slate-800",  size: "text-sm lg:text-base",   pos: "absolute top-[8%] right-[8%] w-7 h-7 lg:w-9 lg:h-9 hidden sm:flex", dur:'4.2s', delay:'0.8s' },
                { Icon: SiFigma,       color: "text-pink-500",   size: "text-sm lg:text-base",   pos: "absolute bottom-[6%] left-[6%] w-7 h-7 lg:w-9 lg:h-9 hidden sm:flex", dur:'3.2s', delay:'2s' },
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2">
        ...
        <RiArrowDownLine className="text-primary text-base animate-bounce" />
      </div>
    </section>
  );
}

...
const webSkills: Skill[] = [
  ...
  { name: "GSAP", icon: SiGreensock, level: 82, color: "#88CE02" },
  { name: "Git & GitHub", icon: SiGithub, level: 90, color: "#181717" },
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="group relative h-full rounded-[20px] p-[1px] interactive cursor-pointer"
      >
        {/* gradient border sheen */}
        <div
          className="absolute inset-0 rounded-[20px] opacity-60 group-hover:opacity-100 transition-opacity duration-500"
          style={{
        ...
        {/* mouse-follow glow */}
        <motion.div
          className="pointer-events-none absolute -inset-px rounded-[20px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10"
          style={{
            background: useTransform(
              [glowX, glowY],
              ([x, y]) =>
                `radial-gradient(280px circle at ${x} ${y}, ${skill.color}35, transparent 70%)`
            ),
          }}
        />

        <div
          className="relative h-full flex flex-col gap-3 sm:gap-4 rounded-[19px] p-3.5 sm:p-4
                     bg-white/45 backdrop-blur-2xl
                     shadow-[0_1px_1px_rgba(255,255,255,0.6)_inset,0_10px_28px_-16px_rgba(15,45,90,0.25)]
                     group-hover:shadow-[0_1px_1px_rgba(255,255,255,0.7)_inset,0_16px_36px_-16px_rgba(15,45,90,0.32)]
        ...
        >
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            <div
              className="relative w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-xl bg-white/80 backdrop-blur flex items-center justify-center
                         shadow-[0_6px_14px_-6px_rgba(15,45,90,0.25)] border border-white/70
                         transition-transform duration-500 group-hover:-translate-y-0.5 group-hover:rotate-3 overflow-hidden"
            >
              {skill.logo ? (
                <img src={skill.logo} alt="" className="w-5 h-5 sm:w-6 sm:h-6 rounded-md" />
              ) : Icon ? (
                <Icon className="text-lg sm:text-xl" style={{ color: skill.color }} />
              ) : null}
              <div
                className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-40 transition-opacity duration-500 blur-md"
                style={{ background: skill.color }}
              />
            </div>
            <h4 className="font-serif font-bold text-sm sm:text-base text-foreground leading-snug truncate">
              {skill.name}
            </h4>
          </div>
          ...
          <div className="mt-auto">
            <div className="flex items-baseline justify-between mb-1.5">
              <span className="text-[9px] uppercase tracking-[0.12em] font-mono text-slate-500/80">
                Proficiency
              </span>
              <span
            ...
            <div className="relative h-1 w-full rounded-full bg-slate-900/[0.06] overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
...
export default function Skills() {
  ...
  return (
    <section
      id="skills"
      className="relative overflow-hidden section-wrap max-w-full pt-20 sm:pt-28 md:pt-36 lg:pt-40 pb-10 sm:pb-14 md:pb-16 lg:pb-20"
          ...
          <div className="lg:col-span-8">
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={activeTab}
                role="tabpanel"
                aria-label={activeTab === "web" ? "Web development skills" : "Video editing skills"}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
